...
\begin{parts}
    ...
    \begin{subparts}
        ...
        \ifans{From $\mathbf{X}_{\text{perm}} = \mathbf{P}\mathbf{X}$, we have 
        \begin{equation*}
            \mathbf{H}_{\text{perm}} =\text{softmax}\left(\frac{\mathbf{P}\mathbf{Q}\mathbf{K}^{\top}\mathbf{P}^{\top}}{\sqrt{d}}\right) \mathbf{P}\mathbf{V} = \mathbf{P}\text{softmax}\left(\frac{\mathbf{Q}\mathbf{K}^{\top}}{\sqrt{d}}\right)\mathbf{V} = \mathbf{P}\mathbf{H}.
        \end{equation*} 
        Therefore, we have
        \begin{align*}
            \mathbf{Z}_{\text{perm}} &= \text{ReLU}(\mathbf{P}\mathbf{H}\mathbf{W}_1 + \mathbf{1}\cdot\mathbf{b}_1)\mathbf{W}_2 + \mathbf{1}\cdot\mathbf{b}_2 = \text{ReLU}(\mathbf{P}\mathbf{H}\mathbf{W}_1 + \mathbf{P}(\mathbf{1}\cdot\mathbf{b}_1))\mathbf{W}_2 + \mathbf{1}\cdot\mathbf{b}_2\\
            &= \mathbf{P}\text{ReLU}(\mathbf{H}\mathbf{W}_1 + \mathbf{1}\cdot\mathbf{b}_1)\mathbf{W}_2 + \mathbf{P}(\mathbf{1}\cdot\mathbf{b}_2) = \mathbf{P} \mathbf{Z}.
        \end{align*}
        Worth mentioning, $\mathbf{1}\cdot\mathbf{b}_2$ is a matrix where each row is exactly the same value, $\mathbf{b}_1$. Therefore, permuting the sequence of $\mathbf{1}\cdot\mathbf{b}_2$ will not change anything.
        }
        ...
        \ifans{This property indicates that when the sequence of tokens in the input sentence is shuffled, the resulting output after the self-attention layer and the feed-forward neural network is simply shuffled in the same way. In other words, the model is permutation-equivariant. The model, without knowing what $\mathbf{P}$ is, cannot distinguish the mapping between input tokens to rows of $\mathbf{P}$. This is problematic as input sentences with different word sequences can have different meanings, and they are represented indistinguishably.}
    \end{subparts}
    ...
    \begin{subparts}
        ...
        \ifans{Yes. With positional embeddings, the token at a specific position $t$ will have a unique sinusoidal embedding added to its word embedding. Therefore, if the input tokens are shuffled, the outputs differ in a way that depends on their position. The model is thus no longer permutation-equivariant, allowing the model to capture word orders.}
        ...
        \ifans{No. For two embeddings to be the same, the position embedding at two positions will have to be exactly the same. This requires 
        \begin{equation*}
            \Phi_{(t', 2i)} = \sin\left({t'}/{10000^{2i/d}}\right) = \sin\left(2n_i\pi+{t}/{10000^{2i/d}}\right)=\Phi_{(t, 2i)}.
        \end{equation*}
        To satisfy the above equation, we need $t'-t$ to be the same integer,
        \begin{equation*}
            t'-t=2n_i\pi(10000^{2i/d}),  
        \end{equation*}
        for every $i$ (dimension of embedding). At different $i$, the scaling factor $10000^{2i/d}$ grows at a different rate, so it is virtually impossible to have integers $n_i$ to satisfy the above condition for any finite length sequence.
        }
    \end{subparts}
\end{parts}
