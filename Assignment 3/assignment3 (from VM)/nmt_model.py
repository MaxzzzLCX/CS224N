...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # Pack means to compress data so the padding is ignored during computation.
        # This is useful as RNN only accepts same length batches, so we have to pad shorter sequences with padding symbols
        X = nn.utils.rnn.pack_padded_sequence(input=X, lengths=source_lengths, batch_first=False, enforce_sorted=True)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X) # LSTM returns hidden_states, (last_hidden, last_cell)
        # Redos the pack_padded_sequence to put the 0-padding back into hidden states
        enc_hiddens, lens = nn.utils.rnn.pad_packed_sequence(sequence=enc_hiddens, batch_first=False)
        enc_hiddens = torch.permute(enc_hiddens, (1, 0, 2)) # (b, src_len, h*2)

        # Separate forward and backward; concatenate (Reshape not ensure data ordering, so no) 
        backward_last_hidden = last_hidden[0,:,:]
        forward_last_hidden = last_hidden[1,:,:]
        last_hidden = torch.cat((backward_last_hidden, forward_last_hidden), dim=1)
        init_decoder_hidden = self.h_projection(last_hidden)
        backward_last_cell = last_cell[0,:,:]
        forward_last_cell = last_cell[1,:,:]
        last_cell = torch.cat((backward_last_cell, forward_last_cell), dim=1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens) # h^{enc}_{proj} = W_{att} @ h^{enc}
        # (b, src_len, h)

        Y = self.model_embeddings.target(target_padded) # Embedding of (actual) target sentence
        # (tgt_len, b, e)
        
        for y_t in torch.split(Y, 1, dim=0): # iterate over 0th element (time index)
            y_t = torch.squeeze(y_t, dim=0) # Turn 3D into 2D; (1, b, e) -> (b, e)
            ybar_t = torch.cat((y_t, o_prev), dim=1) # o_prev (b, h); so cat(y, o) -> (b, e+h)
            dec_state, o_t, e_t = self.step(ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks) # step is below; runs one step of computation of decoder LSTM
            # dec_state = (h_t+1, c_t+1), o = o_t, e = e_t (not used)

            combined_outputs.append(o_t)
            o_prev = o_t
        

        # Stack the list of o_t into a single tensor
        combined_outputs = torch.stack(combined_outputs, dim=0) # o_{1:t}
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # LSTMCell takes in (y_t, (h_t-1, c_t-1))
        dec_state = self.decoder(Ybar_t, dec_state) 
        dec_hidden, dec_cell = dec_state[0], dec_state[1]

        # dec_hidden (b, h); enc_hiddens_proj (b, src_len, h) (this is projected from enc_hiddens via the W_{attPoj})
        # bmm needs two 3D matrices, whose first dimension are both b
        dec_hidden = torch.unsqueeze(dec_hidden, dim=2) # (b, h) -> (b, h, 1)

        e_t = torch.bmm(enc_hiddens_proj, dec_hidden) # e_t (b, src_len, 1)
        e_t = torch.squeeze(e_t, dim=2) # (b, src_len, 1) -> (b, src_len)
        dec_hidden = torch.squeeze(dec_hidden, dim=2) # (b,h,1) -> (b,h)
        ...
        # alpha_t (b, src_len); enc_hiddens (b, src_len, 2h) ==> a_t (b, 2h)
        alpha_t = torch.unsqueeze(alpha_t, dim=1) # (b, src_len) -> (b, 1, src_len)
    ...
    def load(model_path: str):
        ...
        """
        params = torch.load(model_path, weights_only=False, map_location=lambda storage, loc: storage)
        args = params['args']
