...
\begin{parts}
    ...
    \color{blue} $[$ROOT, presented, my] & \color{blue}[findings, at, the, NLP, conference] & \color{blue}presented$\to$I &  \color{blue}\texttt{SHIFT}  \\
    \color{blue}$[$ROOT, presented, my, findings] & \color{blue}[at, the, NLP, conference] & \color{blue}presented$\to$I &  \color{blue}\texttt{SHIFT}  \\
    \color{blue}$[$ROOT, presented, findings] & \color{blue}[at, the, NLP, conference] & \color{blue}findings$\to$my &  \color{blue}\texttt{LEFT-ARC}  \\
    \color{blue}$[$ROOT, presented] & \color{blue}[at, the, NLP, conference] & \color{blue}presented$\to$findings &  \color{blue}\texttt{RIGHT-ARC}  \\
    \color{blue}$\vdots$ & \color{blue}$\vdots$ & \color{blue}$\vdots$ & \color{blue}$\vdots$ \\
    \color{blue}$[$ROOT] & \color{blue}[] & \color{blue}ROOT$\to$presented &  \color{blue}\texttt{RIGHT-ARC}  \\
    \end{tabular}\newline
    \end{adjustwidth}
    ...
    \textcolor{blue}{
        \textbf{Solution}
        Each word has to be "shifted" into the stack from the buffer once, and has to be removed from the buffer as a dependent of some other word once. Therefore, the total number of steps needed is $\boxed{2n}$.
    }
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        \textcolor{blue}{
            \textbf{Solution}
            For some specific $\bh_i$ and $\bx_j$, we have 
            \begin{equation*}
                \bh_i = \relu\left(\bx \bw_i + (\bb_1)_i\right),
            \end{equation*}
            where $\bw_i$ is the $i$th column of matrix $\bW$. Therefore, we have 
            \begin{equation*}
                \frac{\partial \bh_i}{\partial \bx_j} = 
                \begin{cases}
                    \bW_{ji} & \text{if } (\bx\bW + \bb_1)_i > 0, \\
                    0      & \text{otherwise}.
                \end{cases}
            \end{equation*}
        }
        ...
        \textcolor{blue}{
            \textbf{Solution}
            \begin{align*}
                \frac{\partial J}{\partial \bl_i} &= - \frac{\partial}{\partial \bl_i} \sum \by_j \log(\hat{\by}_j) = -\frac{\partial}{\partial \bl_i} \log(\by_c)=-\frac{\partial}{\partial \bl_i} \log(\smx(\bl_c)) \\
                &= -\frac{\partial}{\partial \bl_i} \log\left(\frac{\exp(\bl_c)}{\sum_j \exp(\bl_j)}\right) = -\frac{\partial}{\partial \bl_i} \bl_c + \frac{\exp(\bl_c)}{\sum_j \exp(\bl_j)} \\
                &= 
                \begin{cases}
                    \hat{\by}_i     & \text{if } i \ne c \\
                    \hat{\by}_i + 1 & \text{if } i = c
                \end{cases}
            \end{align*}
        }
...
\textcolor{blue}{
    \textbf{Solution}
    Part-of-speech tags give the parser syntactic category information (e.g., noun, verb, proposition, etc.), which helps it predict likely dependency relations. Certain dependency patterns are strongly correlated with POS; for example, adjectives typically modify nouns. Therefore, by including the POS tags, the parser doesn't have to infer these categories from words alone and can directly utilize such information to make predictions. 
}
